<template>
  <footer class="text-white">
    <div
      class="my-4 grid grid-cols-4 rounded-md border border-gray-200 bg-white p-4"
    >
    ...
    <div class="bg-[#1F032F]">
      <UContainer
        class="mx-auto flex flex-col items-center justify-between gap-6 px-4 py-4 md:flex-row md:gap-0"
      >
        ...
        <div class="flex flex-col items-center space-x-2 rtl:space-x-reverse">
          ...
          <div class="flex space-x-2 rtl:space-x-reverse">
            <NuxtLink
              v-for="social in footer?.social_links?.filter(
                (link) => link.show
              )"
              :key="social.link?.url"
              :to="social.link?.url || '#'"
              :external="social.link?.is_external"
              class="rounded-md bg-gray-700 p-2 hover:bg-gray-600"
            >
              <component
                :is="getSocialIcon(social)"
    ...
    <!-- Main Footer Content -->
    <div
      class="flex min-h-[400px] items-center justify-center bg-white text-gray-800"
    >
      <UContainer>
        <div
          class="grid grid-cols-1 items-start gap-8 md:grid-cols-2 lg:grid-cols-8"
        >
          <!-- Services Column -->
          <div
          ...
          <div class="col-span-1 md:col-span-2 lg:col-span-3">
            ...
            <div class="">
              <h4
                class="mt-4 mb-2 text-center text-lg font-bold text-neutral-800 lg:text-right"
              >
                آدرس فروشگاه
              </h4>
              ...
              <!-- Trust Badges -->
              <div
                class="mt-8 flex items-center justify-center space-x-4 rtl:space-x-reverse"
              >
                <div
                  v-for="badge in footer?.logoes"
                  :key="badge.name"
                ...
                >
                  <NuxtLink
                    v-if="badge.link?.url"
                    :to="badge.link.url"
                    :external="badge.link.is_external"
                  >
                    <img
                      v-if="badge.img"
                      :src="badge.img"
