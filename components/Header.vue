  ...
  <header class="mb-5 border-b border-gray-200 !bg-white px-4">
    <UContainer>
      ...
      >
        ...
        <nuxt-link external to="/">
          <div class="flex items-center justify-center md:justify-start">
            <NuxtImg
              src="logo.svg"
              alt="logo"
              class="ml-2 w-[60px] md:w-[81px]"
            />
            <div>
              <p
                class="text-secondary-300 text-xl leading-6 font-bold md:text-2xl lg:text-4xl"
              >
                محمد
              </p>
        ...
        <!-- User Actions -->
        <div
          class="order-2 hidden w-full md:order-3 md:block md:w-auto md:space-y-4"
        >
          ...
          <div class="hidden items-center justify-between gap-4 md:flex">
            <UModal
              ...
              @update:open="authStore.setLoginModal"
              v-if="!hasToken"
            >
              <UButton
                color="primary"
    ...
    children: category?.children?.map((child) => ({
      ...
      children: child?.children?.map((subChild) => ({
        ...
        children: subChild?.children?.map((subSubChild) => ({
          ...
          to: `/category/${subSubChild?.slug || subSubChild.id}`,
          status: subSubChild.status,
          children: subSubChild?.children?.map((subSubSubChild) => ({
            id: subSubSubChild.id,
            label: subSubSubChild.title,
            to: `/category/${subSubSubChild?.slug || subSubSubChild.id}`,
            status: subSubSubChild.status,
          })),
        })),
      })),
    })),
