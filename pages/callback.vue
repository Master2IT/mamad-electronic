      ...
      <UCard class="text-center">
        ...
        <div class="space-y-4">
          ...
          <div class="flex flex-col gap-3 mt-6">
            ...
            <UButton
              v-if="status === 'failed' || status === 'declined'"
              color="gray"
              variant="soft"
              size="lg"
              class="w-full justify-center"
...
const status = computed(() => {
  ...
  if (queryStatus === 'declined' || queryStatus === 'canceled' || queryStatus === 'cancelled') {
    return 'declined'
  } else if (queryStatus === 'failed' || queryStatus === 'error') {
    return 'failed'
  } else if (queryStatus === 'success' || queryStatus === 'successful' || queryStatus === 'ok') {
    return 'success'
  }
